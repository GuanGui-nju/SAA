...
class FlexMatch:
    ...
    def train(self, args, logger=None):
        ...
        for epoch_i in range(4096):
            # self.loader_dict['train_ulb'].dataset.set_iters(epoch_i)
            # self.print_fn('>> epoch--{}>>--easy_mask--{}'.format(epoch_i, torch.sum(self.loader_dict['train_ulb'].dataset.easy_mask)))
            ...
            for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
                ...
                with amp_cm():
                    ...
                    unsup_loss, mask, select, pseudo_lb, p_model, it_loss = consistency_loss(logits_x_ulb_s,
                                                                                    logits_x_ulb_w,
                                                                                    classwise_acc,
                                                                                    p_target,
                                                                                    p_model,
                    ...
                    self.loader_dict['train_ulb'].dataset.update_loss(x_ulb_idx, it_loss.detach().cpu())
                    del it_loss
